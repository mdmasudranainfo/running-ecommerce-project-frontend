...
const FeatureProducts = ({ product }) => {
  const { cartItems, setCartItems, setPromoDiscount, shopName } = useStatus();
  ...
  useEffect(() => {
    const getData = async () => {
      const res = await request(
        `product/feature-products?page=${page}&limit=20&userType=CUSTOMER`
      );
    ...
    <>
      ...
      <div className="bg-[#ddd] pt-14 xls:pt-4 xms:pt-4 xs:pt-4">
        <div className="max-w-7xl md:max-w-[62rem] sm:max-w-[46rem] xls:max-w-[25rem] xms:max-w-[22rem] xs:max-w-[19rem] mx-auto min-h-[500px]">
          ...
          {loading ? (
            <div className="grid grid-cols-6 gap-x-6 bg-white p-3">
